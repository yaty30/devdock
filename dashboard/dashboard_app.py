...
class DashboardApp:
    def __init__(self, root: tk.Tk):
        ...
        self.processes = {}
        self.service_ready = {"wildfly": False, "vite": False}
    ...
    def reload_app_config(self):
        ...
        self.enqueue("wildfly", f"\n[{time.strftime('%H:%M:%S')}] Settings saved. Restart services to use changed commands.\n")
    ...
    def _start_services(self):
        self.start_service("wildfly", WILDFLY_COMMAND, WILDFLY_DIR, self.wildfly_pane)
        self.start_service("vite", FRONTEND_COMMAND, FRONTEND_DIR, self.vite_pane)

    def start_service(self, key: str, command: str, cwd: str, pane: TextPane):
        with self.process_lock:
            self.service_ready[key] = False
        pane.set_status("Starting", YELLOW)
        self.enqueue(key, f"[{time.strftime('%H:%M:%S')}] Starting: {command}\n")
        self.enqueue(key, f"[{time.strftime('%H:%M:%S')}] Working directory: {cwd}\n")
        t = threading.Thread(target=self._run_process_thread, args=(key, command, cwd, pane, False), daemon=True)
        t.start()

    def restart_service(self, key: str, command: str, cwd: str, pane: TextPane):
        self.stop_named_process(key)
        pane.clear()
        self.start_service(key, command, cwd, pane)
    ...
    def start_build(self, builder: dict):
        ...
        with self.build_lock:
            ...
            t = threading.Thread(target=self._run_process_thread, args=("build", command, cwd, self.build_log_pane, True), daemon=True)
            t.start()
    ...
    def stop_named_process(self, key: str):
        with self.process_lock:
            self.service_ready[key] = False
    ...
    def _run_process_thread(self, key: str, command: str, cwd: str, pane: TextPane, is_build: bool):
    ...
    def _start_log_tail(self):
        threading.Thread(target=self._tail_log_path_worker, daemon=True).start()

    def _tail_log_path_worker(self):
        ...
        while not self.stop_event.is_set():
            try:
                desired_path = os.path.expandvars(os.path.expanduser((LOG_PATH_FILE or "").strip().strip('"')))
    ...
    def flush_queues(self):
        ...
        for key, pane in pane_map.items():
            ...
            try:
                while True:
                    piece = self.queues[key].get_nowait()
                    pieces.append(piece)
    ...
    def refresh_timers(self):
        ...
        with self.process_lock:
            wildfly_running = "wildfly" in self.processes
            wildfly_ready = self.service_ready.get("wildfly", False)
            vite_running = "vite" in self.processes
            vite_ready = self.service_ready.get("vite", False)

        if wildfly_running:
            self.wildfly_pane.set_status("Running" if wildfly_ready else "Starting", GREEN if wildfly_ready else YELLOW)
        elif self.wildfly_pane.status_var.get().startswith("Starting"):
            self.wildfly_pane.set_status("Stopped", MUTED)

        if vite_running:
            self.vite_pane.set_status("Running" if vite_ready else "Starting", GREEN if vite_ready else YELLOW)
